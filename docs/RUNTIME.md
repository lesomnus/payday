...
| 4i | 생성물과 런타임의 payday 버전이 같다 | 생성된 파일에 버전을 박고 런타임이 불일치를 **거부** (§12.1) | 없음. 어긋나면 조용히 틀린다 |
| 4j | 서버는 스키마와 다른 데이터베이스 위에서 뜨지 않는다 | 시작할 때 확인하고 거절 (§12.1) | 없음. 업그레이드 후 `migrate plan`을 빠뜨리는 것이 기본 실수가 된다 |
