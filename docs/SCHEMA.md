...
3번을 선언하면 `pd.Grouped`가 나온다. 엔티티마다 술어 하나씩, **집합에 없는 것까지
포함해서 전부**.
...
`of`는 이 caller가 어느 집합을 볼 수 있는지 답하는 함수다 — 멤버십 테이블 쿼리라 앱의
것이다. `nil`이어도 되고, 그러면 정책은 아무것도 안 좁힌다.

**손으로 쓰지 않는 이유는 열다섯 줄이 어려워서가 아니다.** 손으로 쓰면 `bare.Unscoped`를
임베드하고 할 말이 있는 엔티티만 쓰게 되는데, 그러면 **나중에 추가된 엔티티가 좁혀지지
않고 아무것도 그 사실을 말하지 않는다.** 벽을 생성하는 이유와 같은 구멍이고, 같은 자리에
난다 — 그리고 구멍 난 두 번째 축은 없는 것보다 나쁘다. 첫 번째 축이 일을 하고 있는 것처럼
보이기 때문이다.

셋이 중요하다.

- **한 홉이다.** 행은 자기 3번이 가리키는 집합에 있고, 3번이 없는 엔티티는 안 좁혀진다.
  `via:`처럼 엣지를 따라 걸어나가지 않는다 — 테넌트는 구조상 모든 것에서 닿지만 집합은
  아니다. 자식 행이 집합에 속하면 그 행에 3번을 단다.
...
3번을 **nullable로 둘 수 있고**, 그러면 집합에 없는 행이 생긴다. 그 행은 집합으로 좁혀진
읽기에 **안 보인다.** 방향은 맞다 — 아무도 site에 넣지 않은 행이 모든 site에 있는 것보다
낫다 — 다만 이미 행이 있는 스키마에 3번을 추가하면 백필 전까지는 site 자격증명으로 아무도
못 보게 된다는 뜻이다.

#### 자격증명도 집합으로 좁혀진다

`frame.Grant`의 세 번째 축이다.

```go
frame.Whole().In(tenant).Within(site)   // 이 테넌트의 이 site 전용 키
```

site에 매단 API 키가 이걸 필요로 한다. 그런 키는 일이 벌어지는 곳에서 발급되고, 발급한
사람보다 오래 살고, **다른 곳에서는 쓸 수 없어야** 한다 — 테넌트 축은 그 말을 못 한다.
테넌트의 모든 site가 그 테넌트 안에 있기 때문이다.

`Grouped`는 `of`를 직접 부르지 않고 `frame.NarrowSet(ctx, of)`을 통해 부른다. `of`는
**행위자**가 무엇을 볼 수 있는지에 답하고, 자격증명이 그중 무엇을 허용하는지는 별개의
질문이라서다. 앱이 첫 질문에만 답하고 두 번째를 생각 안 하면 site 키가 모든 site에
닿는데, 그걸 말해주는 것이 아무데도 없다.

둘 다 있으면 **교집합**이다. 자격증명은 언제나 좁히기만 하고 넓히지 않는다 — 들고 있는
사람이 못 보는 집합에 토큰이 닿지 않는다. 그리고 `of`가 아예 없어도(`nil`) 자격증명은
좁힌다.

`Within()`을 빈 목록으로 부르면 **아무 집합도 허용 안 함**이고 읽는 행이 없다. 반대로
읽으면 목록이 비면서 문이 열리는 것이 된다.
...
#### 2.1.2 `Erase`가 무엇을 하는지는 적어야 한다

**아무 말도 안 하면 soft다.** 다만 payday가 앱 스키마에 필드를 넣을 수는 없으므로,
"침묵 = soft"는 기본값이 아니라 **거절**로 구현된다 — 무엇을 쓸지 알려주면서.

| `erased:` 필드 | `erase: {hard: {}}` | |
| --- | --- | --- |
| 있음 | 없음 | soft |
| 없음 | 있음 | hard |
| 없음 | 없음 | **거절** |
| 있음 | 있음 | **거절** — 필드가 있으면 ORM은 soft로 도는데 스키마는 파괴라고 말한다 |

두 실패가 같지 않아서 기본값이 아니라 거절이다. **soft라고 잘못 가정하면** 없어야 할 행이
남고, 그건 보면 알 수 있다. **hard라고 잘못 가정하면 행이 사라지고**, 그건 누가 돌려달라고
할 때 알게 된다.

soft가 뜻하는 것은 "아무 일도 안 일어났다"가 아니다. 행은 도장이 찍히고 남아서 **읽을 수
없고, 고칠 수 없고, 별칭은 풀린다**(유니크 인덱스가 `WHERE date_erased IS NULL`로 부분화된다).
그리고 남아 있으므로 **트레일이 그 내용을 읽을 수 있다** — `Audit.value`가 그것이다.

hard도 진짜 답이다. 읽는 속도보다 빨리 도착하는 것들(측정값, 이벤트)의 테이블이 아무것도
못 지우는 테이블이 되면 안 되고, payday는 대신 내놓을 보존 정책이 없다. 대가는 트레일이
말할 수 없게 되는 것이다: `Audit.value`는 쓰기 **뒤에** 행을 읽어서 채우므로, 사라진 행에는
읽을 것이 없다.

**대신 데이터베이스에 직접 붙는 것으로 답하지 말 것.** 앱 밖에서 돈 `DELETE`는 트레일도,
버전도, `Watch`도 건너뛴다 — 그리고 watch는 **안 보내는 것으로** 행이 사라졌다고 말하므로,
앱 등 뒤에서 지운 행은 그것을 들고 있는 모든 클라이언트가 영원히 들고 있는 행이 된다.
**선언된 hard가 DB 직접보다 안전하다.**
...
#### payday가 흡수한 것과 안 한 것

**흡수한 것은 둘이다.** 엔티티마다의 술어(`pd.Grouped`)와 자격증명의 축
(`frame.Grant.Within`). 둘 다 빠뜨리면 조용한 것들이라 생성하거나 강제한다.

**안 한 것은 두 번째 축을 1급으로 올리는 것이다.** `gate.ByTenant`의 단일 키, `audit`의
테넌트 컬럼, `slug`의 2단 `@tenant/alias`, `alias`의 테넌트별 유일성 — 전부 여전히 테넌트
하나를 전제한다. 집합은 **읽기를 좁히는 축**이지 이름 공간이 아니다. site마다 같은 별칭을
쓰고 싶다면 그건 이 축이 답하는 질문이 아니다.

그리고 Site가 **배포 단위**라면 [TENANCY.md]의 답이 여전히 더 강하다 — 배포를 나누고,
격리를 술어가 아니라 **행의 부재**로 만드는 것. 술어는 빠뜨릴 수 있고 없는 데이터는
빠뜨릴 수 없다. 두 축이 실제로 일하는 곳은 여러 Site를 한 번에 보는 관제 평면이다.
