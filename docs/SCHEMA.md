...
- **읽기만이다.** `Scope`는 술어이고 `Get`·`List`·`Patch`·`Erase`는 WHERE를 가진
  쿼리이므로 좁혀진다. **`Add`는 아니다** — INSERT에는 WHERE가 없고, 엣지로 주어진
  식별자는 그대로 외래 키가 된다. 그러니 caller가 볼 수 없는 Site에 행을 만드는 것은
  막히지 않는다. 이것은 이 축의 성질이 아니라 **테넌트 벽의 성질과 같다**(§2.2).
  막아야 한다면 레이어에서 `Add`의 엣지를 검사한다.
- **`WithScope`는 두 번 못 준다.** 두 번 주면 `ErrTwice`이고 에러가 `Scopes{...}`를
  가리킨다 — 둘 중 하나를 조용히 잃는 것이 이 자리에서 가장 나쁜 답이기 때문이다.

#### 2.2 벽은 읽기의 것이다 — `Add`는 좁혀지지 않는다

위 절이 `Scope`에 대해 말한 것은 **테넌트 벽에도 그대로**이고, 그것이 지금 payday의
상태다. 실제로 확인했다:

```
@acme/admin  →  AssetService.Add {tenant: {alias: "other"}, ...}   → 성공한다
@acme/admin  →  AssetService.List                                  → 그 행이 안 보인다
@acme/admin  →  TenantService.Get {alias: "other"}                 → NotFound
```

읽기는 막히고 **쓰기는 안 막힌다.** `Wall()`의 주석이 "puts every read behind the
tenant"라고 정직하게 적고 있으므로 거짓말은 아니지만, 읽는 사람은 벽이라는 말에서
양방향을 기대한다.
...
| | 좁혀지나 |
| --- | --- |
| `Get`·`List`·`Watch` | **예** — WHERE가 붙는다 |
| `Patch`·`Apply`·`Erase` | **예** — 대상을 고르는 것이 WHERE다 |
| `Add` | **아니오** — INSERT에 WHERE가 없고, 엣지의 식별자가 그대로 외래 키가 된다 |

읽기 유출은 아니다. 남의 데이터를 보게 되지는 않는다. 되는 것은 **남의 테넌트에 행을
심는 것**이고, 대가는 셋이다 — 피해자가 자기 것으로 읽는 데이터, 사용량, 그리고 그 쓰기의
감사 행이 **행위자의 테넌트**에 찍히므로 피해자의 트레일에는 아무 일도 안 남는다는 것.

막으려면 지금은 레이어다. 생성된 `Add` 앞에서 엣지로 주어진 식별자가 `frame.Narrow`가
답한 것 안에 있는지 보는 것이고, 레이어는 배치에서도 다시 적용되므로 한 곳이면 된다.
payday가 이것을 생성하지 않는 것은 결정이라기보다 **아직 하지 않은 것**이다.
