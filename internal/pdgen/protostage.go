...
// TsSource is where an entity's generated TypeScript landed.
type TsSource struct {
	// Msg is the module holding the message schema.
	Msg string

	// Svc is the module holding the service, and empty for an entity that is
	// stored and not served.
	Svc string
}

func TsEntities(s *Schema, at func(*Entity) TsSource) string {
	b := &strings.Builder{}
...
`)
	...
	// entities share a file often enough for it to be the normal case. The
	// services are the same again and are a second group: they are in other
	// files, which is the whole reason the declaration has to carry them.
	src := map[*Entity]TsSource{}
	for _, v := range s.Sorted() {
		src[v] = at(v)
	}

	group := func(pick func(TsSource) string, suffix string) {
		...
		for _, p := range order {
			fmt.Fprintf(b, "import { %s%s } from '%s'\n",
				strings.Join(seen[p], suffix+", "), suffix, p)
		}
	}

	group(func(v TsSource) string { return v.Msg }, "Schema")
	group(func(v TsSource) string { return v.Svc }, "Service")
...
// TsImport is the module a `.proto` was generated into, relative to the root of
// the generated tree -- protoc-gen-es keeps the directory and renames the file.
//
// The name is cut at the **first** dot rather than at the `.proto` suffix,
// because that is what protoc-gen-es does: `robot_svc.g.proto` is
// `robot_svc_pb.ts`, and the `.g` survives only in the symbol it exports
// (`file_app_robot_svc_g`). Trimming the suffix instead agrees with it on every
// name that has one dot, which is every file this was used for until the
// service contracts -- which are `.g.proto`, always.
//
// It is a rule mirrored rather than chosen, so it is written here once and the
// test beside it is what says the two still agree.
func TsImport(p string) string {
	...
	return "./" + path.Join(path.Dir(p), base+"_pb.js")
}
