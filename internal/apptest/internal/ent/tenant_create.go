...
func (_c *TenantCreate) sqlSave(ctx context.Context) (*Tenant, error) {
	...
	if _spec.Id.Value != nil {
		...
		if !ok {
			sv = tenant.ValueScanner.Id.ScanValue()
			if err := sv.Scan(_spec.Id.Value); err != nil {
				return nil, err
			}
		}
		if value, err := tenant.ValueScanner.Id.FromValue(sv); err != nil {
			return nil, err
		} else {
			_node.Id = value
		}
...
func (_c *TenantCreate) createSpec() (*Tenant, *sqlgraph.CreateSpec, error) {
	...
	if id, ok := _c.mutation.Id(); ok {
		_node.Id = id
		vv, err := tenant.ValueScanner.Id.Value(id)
		if err != nil {
			return nil, nil, err
		}
		_spec.Id.Value = vv
		...
		func(i int, root context.Context) {
			...
			var mut Mutator = MutateFunc(func(ctx context.Context, m Mutation) (Value, error) {
				...
				if specs[i].Id.Value != nil {
					...
					if id, err := tenant.ValueScanner.Id.FromValue(sv); err != nil {
						return nil, err
					} else {
						nodes[i].Id = id
					}
