...
// Mutate calls f(ctx, m).
func (f CellFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.CellMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.CellMutation", m)
}
...
// function as Joint mutator.
type JointFunc func(context.Context, *ent.JointMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f JointFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.JointMutation); ok {
		return f(ctx, mv)
	}
