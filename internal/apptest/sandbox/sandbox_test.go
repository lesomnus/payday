...
func rowsOf(ctx context.Context, b *strings.Builder, db *sql.DB, t string) error {
	...
	for i, c := range cs {
		qs[i] = fmt.Sprintf("quote(%s)", quoted(c))
		ns[i] = quoted(c)
	}
	...
	for rows.Next() {
		...
		fmt.Fprintf(b, "INSERT INTO %s (%s) VALUES (%s);\n",
			quoted(t), strings.Join(ns, ", "), strings.Join(vs, ", "))
	}

	return rows.Err()
}

func columnsOf(ctx context.Context, db *sql.DB, t string) ([]string, error) {
	rows, err := db.QueryContext(ctx, fmt.Sprintf(`SELECT name FROM pragma_table_info(%s)`, literal(t)))
	if err != nil {
		return nil, err
	}
	defer rows.Close()
